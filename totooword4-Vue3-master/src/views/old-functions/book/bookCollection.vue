...
import { listBook,listByUserBookBySelf, getBook, delBook, addBook, updateBook } from "../../../api/functions/book.js";
...
function handleUpdate(row) {
  ...
  getBook(_bookId).then(response => {
    ...
    funUserBookList.value = response.data.funUserBookList;
    open.value = true;
    title.value = "修改书本信息";
  });
}
...
function submitForm() {
  proxy.$refs["bookRef"].validate(valid => {
    if (valid) {
      form.value.funUserBookList = funUserBookList.value;
...
/** 用户收藏书本添加按钮操作 */
function handleAddFunUserBook() {
  let obj = {};
  obj.collectionTime = "";
  obj.collectionNote = "";
  obj.isOwned = "";
  obj.acquisitionMethod = "";
  funUserBookList.value.push(obj);
}

/** 用户收藏书本删除按钮操作 */
function handleDeleteFunUserBook() {
  if (checkedFunUserBook.value.length == 0) {
    proxy.$modal.msgError("请先选择要删除的用户收藏书本数据");
  } else {
    const funUserBooks = funUserBookList.value;
    const checkedFunUserBooks = checkedFunUserBook.value;
    funUserBookList.value = funUserBooks.filter(function(item) {
      return checkedFunUserBooks.indexOf(item.index) == -1
    });
  }
}
