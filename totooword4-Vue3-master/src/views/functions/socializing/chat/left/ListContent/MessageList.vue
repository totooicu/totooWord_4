...
const getAvatar = (item) => {
  ...
  if (item.messageType==="1") {
    ...
    if(item.senderId===userStore.$state.id)return item.receiver.avatar || ''; // 好友头像
    else  return item.sender.avatar || ''; // 好友头像
  }
  return '';
};
...
// 获取名称
const getName = (item) => {
  console.log("获取名称item",item)
  if (item.messageType==="1") {
    return item.group.groupName; // 群聊名称
  } else if (item.messageType==="0") {
    let selfid=userStore.$state.id
    if(item.senderId===selfid)return item.receiver.nickName
    else return item.sender.nickName; // 好友昵称
  }
  return '未知';
};
